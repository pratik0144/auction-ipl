...
export default function JoinRoomWithCodePage({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!displayName.trim() || !squadName.trim()) {
      ...
      <div className="w-full max-w-md animate-slide-up">
        ...
        <div className="card">
          ...
          <p className="text-muted text-sm mb-6">
            Joining room{' '}
            <span className="font-mono text-link">{code.toUpperCase()}</span>
          </p>

          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            {/* Room Code (pre-filled, read-only) */}
            <label className="flex flex-col gap-1.5">
              ...
              <input
                ...
                readOnly
                className="w-full px-3 py-2.5 bg-surface border border-hairline rounded-md text-link font-mono text-xl tracking-widest text-center uppercase cursor-not-allowed opacity-80"
              />
            </label>
