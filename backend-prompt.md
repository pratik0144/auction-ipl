...
3. Authentication: lightweight session identity is enough — full Supabase Auth (email/OAuth) is NOT required, but you may use Supabase Auth's anonymous sign-in if it cleanly solves identity/RLS without overengineering it. Justify whichever you pick.
4. Write **Row Level Security policies** appropriate for this use case (e.g. a participant shouldn't be able to write another participant's bid, shouldn't be able to directly mutate budgets/squad counts client-side — that must only happen through validated server-side logic).
